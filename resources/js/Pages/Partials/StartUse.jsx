...
export default function StartUse({vehicles, vehicle, vehicle_usages, requests_approved}) {
        ...
        return (
            <Dialog open={isDialogOpen} onOpenChange={handleDialogClose}>
                ...
                <DialogContent className="max-w-xl">
                    ...
                    <StartUseForm vehicle_usages={vehicle_usages} vehicles={vehicles} data={data} setData={setData} errors={errors} submit={submit} currentPart={currentPart} totalParts={totalParts} setCurrentPart={setCurrentPart} requests_approved={requests_approved} />
                </DialogContent>    
            </Dialog>
        );
    }
    ...
    function StartUseForm({ vehicles, vehicle_usages, vehicle_usage,  data, setData, errors, submit, currentPart, totalParts, setCurrentPart, requests_approved})
    {
        ...
        const setVehicle = (vehicle_uuid, vehicle_usage_uuid) => (e) => {
            const vehicle = vehicles?.find(v => v.id === vehicle_uuid);
            
            const date = new Date().toLocaleString('en-My');
            console.log(date);
            
            setData('current_odometer', vehicle ? vehicle.current_odometer : '');
            setData('vehicle_usage_uuid', vehicle_usage_uuid);
            setData('actual_start_datetime', date ? date : '');
            ...
            <div className='p-4'>
                <form onSubmit={submit}>
                    <div className="items-center space-y-2">
                        {currentPart === 1 && (
                        <div>
                            <div className="grid flex-2 gap-2 md:grid-flex-2 my-2">
                                {requests_approved.map((request_approved) => (
                                <div key={request_approved.id} onClick={setVehicle(request_approved.vehicle_uuid, request_approved.id)}>
